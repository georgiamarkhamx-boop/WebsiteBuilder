...
export class MemStorage implements IStorage {
  ...
  async createUser(insertUser: InsertUser): Promise<User> {
    ...
    const user: User = { 
      ...
      isActive: true,
      trialStartDate: (insertUser as any).trialStartDate || new Date(),
      trialEndDate: (insertUser as any).trialEndDate || null,
      isTrialExpired: (insertUser as any).isTrialExpired || false,
      lastTrialNotification: null,
      mfaSecret: null,
      mfaEnabled: false,
      lastLogin: null,
      loginAttempts: 0,
